...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.state:
            output = F.upsample(output, input.size()[2:], None, 'bilinear')
...
class ENet_ENC(nn.Module):
    def __init__(self, n_classes=19, only_encode=True):
        super(ENet_ENC, self).__init__()
        self.state = only_encode
        self.encoder = Encoder(n_classes, only_encode=only_encode)
        # self.decoder = Decoder(n_classes)
...
class EACNet_ENet(nn.Module):
    def __init__(self,n_classes=19, only_encode=False):
        super(EACNet_ENet, self).__init__()
        self.state = only_encode
        self.encoder = Encoder(n_classes, only_encode=only_encode)
        self.classify = nn.Conv2d(128, n_classes, 1)
        self.eac = EAC_Module(n_classes, n_classes)
